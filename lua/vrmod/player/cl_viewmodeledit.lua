...
if CLIENT then
	...
	local DEFAULT_VIEWMODEL_INFO = {
		-- Extra nudge added on top of every auto-computed hand-bone offset.
		-- The old (0, 0.2, 0) fudge pushed guns forward out of the hand; auto
		-- offsets now trust the bone. Still overridable via saved config.
		autoOffsetAddPos = Vector(0, 0, 0),
		gmod_tool = {
	...
	-- LoadViewModelConfig expects offsets as {x, y, z} arrays, but TableToJSON
	-- serializes Vector/Angle userdata to "[x y z]" STRINGS the loader rejects
	-- — saved offsets never actually restored. Serialize as arrays explicitly.
	local function VecArr(v) return v and {v.x, v.y, v.z} or nil end
	local function AngArr(a) return a and {a.p, a.y, a.r} or nil end

	local function SaveViewModelConfig()
		-- Persist only what the loader reads back: closedHandAngles etc. are
		-- recomputed on every weapon switch (pure bloat on disk), and
		-- auto-computed offsets must not be frozen — a bad automatic value
		-- would override the per-session recompute forever. User-set offsets
		-- (autoComputed cleared by SetViewModelOffsetForWeaponClass) do persist.
		local out = { autoOffsetAddPos = VecArr(g_VR.viewModelInfo.autoOffsetAddPos) }
		for cls, v in pairs(g_VR.viewModelInfo) do
			if istable(v) then
				local keep = not v.autoComputed
				local c = {
					offsetPos = keep and VecArr(v.offsetPos) or nil,
					offsetAng = keep and AngArr(v.offsetAng) or nil,
					wrongMuzzleAng = v.wrongMuzzleAng,
					noLaser = v.noLaser
				}
				if next(c) then out[cls] = c end
			end
		end
		file.Write(CONFIG_PATH, util.TableToJSON(out, true))
	end
