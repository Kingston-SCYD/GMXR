...
local function buildClientFrame(relative)
	...
	-- Resolved by sh_trackers.lua, which falls back to these same three keys
	-- when nothing is configured, so a plain 3-puck rig is unchanged.
	local fbt = g_VR.fbtPose
	local waistPose     = fbt and fbt.pelvis    or tracking.pose_waist
	local leftFootPose  = fbt and fbt.leftfoot  or tracking.pose_leftfoot
	local rightFootPose = fbt and fbt.rightfoot or tracking.pose_rightfoot
	if waistPose and leftFootPose and rightFootPose then
		frame.waistPos = waistPose.pos
		frame.waistAng = waistPose.ang
		frame.leftfootPos = leftFootPose.pos
		frame.leftfootAng = leftFootPose.ang
		frame.rightfootPos = rightFootPose.pos
		frame.rightfootAng = rightFootPose.ang
	end
