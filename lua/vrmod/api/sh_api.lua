...
function vrmod.AddCallbackedConvar(cvarName, valueName, defaultValue, flags, helptext, min, max, conversionFunc, callbackFunc)
	...
	if not cv then
		if CLIENT and bit.band(flags, FCVAR_REPLICATED) == 0 then
			cv = CreateClientConVar(cvarName, tostring(defaultValue), true,
				bit.band(flags, FCVAR_USERINFO) ~= 0, helptext or "", min, max)
		else
			cv = CreateConVar(cvarName, defaultValue, flags, helptext, min, max)
		end
	end
	convars[cvarName] = cv
...
-- Visual debug system: all VR debug traces/wireframes check this
if CLIENT then
    CreateClientConVar("vrmod_visual_debug", "0", true, FCVAR_ARCHIVE, "Show all VR debug visuals", 0, 1)
end

function vrmod.DebugVisible()
    if SERVER then return false end
    local cv = GetConVar("vrmod_visual_debug")
