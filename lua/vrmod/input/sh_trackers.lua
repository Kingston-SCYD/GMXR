...
-- stage, so after the axes are right there can still be a heading and a height
-- to dial out. vrmod_trackers_diag prints what you need to set these.
vrmod.AddCallbackedConvar("vrmod_trackers_oscyaw", "trackersOscYaw", "0", FCVAR_ARCHIVE,
	"Heading correction for external trackers, in degrees", -180, 180, tonumber)
vrmod.AddCallbackedConvar("vrmod_trackers_oscheight", "trackersOscHeight", "0", FCVAR_ARCHIVE,
	"Height correction for external trackers, in metres", -2, 2, tonumber)
...
local function Rescan()
	...
	-- Count of trackers ELIGIBLE for FBT. This is what cl_vrmod.lua turns into
	-- g_VR.sixPoints, which is what gates the Calibrate Full-body Tracking menu
	-- item.
	--
	-- Deliberately NOT the count of resolved slots below. Resolution needs an
	-- assignment; the assignment is what calibration produces; and calibration
	-- is reached through that menu item. Gating on resolution means an OSC rig
	-- can never reach the calibration that would resolve it -- the menu entry
	-- simply never appears, which is exactly the bug this replaced.
	local eligible = 0
	for i = 1, #order do
		local s = order[i]
		if s.label and s.label ~= "" then labelIndex[string.lower(s.label)] = s end
		if s.role ~= "object" and s.role ~= "off" and IsLive(s) then
			eligible = eligible + 1
		end
	end
	g_VR.fbtTrackerCount = eligible
	...
	-- How many slots are actually driving the body right now. Reported by
	-- vrmod_trackers_diag; not a gate on anything.
	g_VR.fbtResolvedCount = n
end
...
local TrackerSetOrigin = vrmod.TrackerSetOrigin or VRMOD_TrackerSetOrigin
local lastOriginYaw, lastOriginHeight

--- Pushes the origin correction down to the module. Cheap enough to poll, and
--- polling means the convars can be swept live while watching the debug cubes.
---
--- Defined above Poll on purpose: a `local function` is only in scope after its
--- own declaration, so calling it from a closure created earlier resolves to
--- the global instead and errors at runtime.
local function ApplyOrigin(force)
	if not TrackerSetOrigin then return end
	local y, h = cv.trackersOscYaw or 0, cv.trackersOscHeight or 0
	if not force and y == lastOriginYaw and h == lastOriginHeight then return end
	lastOriginYaw, lastOriginHeight = y, h
	TrackerSetOrigin(y, h)
end
...
-- Everything you need to work out whether a misplaced tracker is a heading
-- error, a height error, or a scale error. Print it standing upright, facing
-- the direction you faced when you last did a SlimeVR full reset.
concommand.Add("vrmod_trackers_diag", function()
	Rescan()
	local hmd = g_VR.tracking and g_VR.tracking.hmd
	if not hmd or not hmd.pos then
		print("[Trackers] no HMD pose -- start VR first")
		return
	end
	print(string.format("HMD world  %.1f %.1f %.1f   yaw %.1f   playspace origin %.1f %.1f %.1f yaw %.1f",
		hmd.pos.x, hmd.pos.y, hmd.pos.z, hmd.ang.y,
		g_VR.origin.x, g_VR.origin.y, g_VR.origin.z, g_VR.originAngle.y))
	print(string.format("corrections: yaw %.1f deg, height %.2f m, scale %.2f u/m",
		cv.trackersOscYaw or 0, cv.trackersOscHeight or 0, g_VR.scale or 0))
	print(string.format("FBT: %d eligible, %d slots resolved, sixPoints %s%s",
		g_VR.fbtTrackerCount or 0, g_VR.fbtResolvedCount or 0,
		tostring(g_VR.sixPoints and true or false),
		(g_VR.fbtTrackerCount or 0) >= 3 and (g_VR.fbtResolvedCount or 0) < 3
			and "  (calibrate to resolve the slots)" or ""))
	print(string.format("%-16s %-22s %-22s %s", "tracker", "world", "relative to HMD", "raw (playspace, m)"))
	for i = 1, #order do
		local s = order[i]
		if IsLive(s) then
			local p = s.pose.pos
			local rel = WorldToLocal(p, angle_zero, hmd.pos, Angle(0, hmd.ang.y, 0))
			local rawStr = "-"
			if s.raw and s.raw.pose then
				local rp = s.raw.pose:GetTranslation()
				rawStr = string.format("%6.2f %6.2f %6.2f", rp.x, rp.y, rp.z)
			end
			print(string.format("%-16s %6.0f %6.0f %6.0f   %6.0f %6.0f %6.0f   %s",
				s.label, p.x, p.y, p.z, rel.x, rel.y, rel.z, rawStr))
		end
	end
	print("Relative columns are forward / left / up from the HMD. A foot tracker")
	print("should read roughly 0 forward, +-6 left, -60 up. If forward and left")
	print("look swapped or negated, set vrmod_trackers_oscyaw. If up is off by a")
	print("constant, set vrmod_trackers_oscheight.")
end)
